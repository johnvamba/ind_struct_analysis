...
function Beam() {
	...
	this.stressesOn = function(x_coor){
		...
		var momentArm = { 
			equally:function(l,ld,x,x2){
				// console.log('dapat',(x-x2)%(l/2)); //nope
				// console.log('dapat',(x-x2)<=(l/2) ? (x-x2): x+x2-l)  //dili sad
				// console.log('dapat3',)
				return (x-x2)<=(l/2) ? (x-x2): x+x2-l
	        // return x - ((l/2) + x2);
	      	},left_con:function(l,ld,x,x2){
	          return x - ((l/3) + x2);//original
	        },right_con:function(l,ld,x,x2){
	          return x - ((2*l/3) + x2); //original
		...
		function dividedMembers(){
			...
			var temp;
			var pushBy = { 
				equally:function(i,y,l,len,tlen){ //done i think //NOPE!.. partly. dapat maaply ang shear.
					if(tlen === undefined){
						console.log('case1 equally');
						console.log(react,'before',react-(y - areaForce['equally'](len,l,len,0)/2),-(y - areaForce['equally'](len,l,len,0)/2));
						react -= y - areaForce['equally'](len,l,len,0)/2;
						console.log(react,'after',y,- areaForce['equally'](len,l,len,0),momentArm['equally'](len,l,len,0));
						// moment -= react * len; 
						console.log('shear',-shear(len),-shearb4elm);
						arr.push({
							xstep:len,
							index:i,
							yreact:react,
							tentMoment: -react * len,
							// arm: momentArm['equally'](len,l,len,0)
						});
					} else {
						console.log('case2 equally');
						console.log(react,'before',react-(y - areaForce['equally'](len,l,x_coor,tlen)/2), -y - areaForce['equally'](len,l,x_coor,tlen)/2);
						react -= y - areaForce['equally'](len,l,x_coor,tlen)/2;
						console.log(react,'after',y,-areaForce['equally'](len,l,x_coor,tlen)/2,momentArm['equally'](len,l,x_coor,tlen));
						// moment -= react * (x_coor-tlen);
						console.log('shear',-shear(x_coor-tlen));
						arr.push({
							xstep:x_coor-tlen,
							index:i,
							yreact:react,
							tentMoment: -react * (x_coor-tlen),
							// shear: -shear(len)
							// arm: momentArm['equally'](len,l,x_coor,tlen)
						});
					} //equally
		     	},left_con:function(i,y,l,len,tlen){
					if(tlen === undefined){
						console.log(react,'before',react-(y-shear(len)));
						// react -= y - areaForce['left_con'](len,l,len,0);//original
						// react -= y - (areaForce['left_con'](len,l,len,0)/2);//original
						react -= y-shear(len);
						// react -= y-((shear()/2)*momentArm['left_con'](len,l,len,0));
						// console.log('shear',(shear()/2)*momentArm['left_con'](len,l,len,0));
						// react -= y - (areaForce['left_con'](len,l,len,0)/2)*momentArm['left_con'](len,l,len,0)/2;
						// react -= y - (y/2) //ni curve siya.. care
						console.log(react,'after',y,- areaForce['left_con'](len,l,len,0)/2,momentArm['left_con'](len,l,len,0));
						arr.push({
							xstep:len,
							index:i,
							yreact:react,
							// arm: momentArm['left_con'](len,l,len,0),
						});
					} else {
						console.log(react,'before',react-(y-shear(x_coor)));
						// react -= y - areaForce['left_con'](len,l,x_coor,tlen);//original
						// react -= y - (areaForce['noload'](len,l,x_coor,tlen)/2);
						// react -= y-((shear()/2)*momentArm['left_con'](len,l,x_coor,tlen));
						react -= y-shear(x_coor);

						// console.log('shear',(shear()/2)*momentArm['left_con'](len,l,x_coor,tlen));
						// react -= y - (areaForce['left_con'](len,l,x_coor,tlen)/2)*momentArm['left_con'](len,l,x_coor,tlen)/2; //original
						// react -= y - (y/2) //ni curve siya.. care
						console.log(react,'after',y,-areaForce['left_con'](len,l,x_coor,tlen)/2,momentArm['left_con'](len,l,x_coor,tlen));
						moment -= react * (x_coor-tlen);
						arr.push({
							xstep:x_coor-tlen,
							index:i,
		        ...
		        },right_con:function(i,y,l,len,tlen){
					if(tlen === undefined){
						react -= y-shear(len);
						// react -= y - (areaForce['right_con'](len,l,len,0)/2);
						console.log(-shear(len));
						arr.push({
							xstep:len,
							index:i,
							yreact:react,
							// shear: -shear(len)
							// arm: momentArm['right_con'](len,l,len,0),
						});
					} else {
						// react -= y - areaForce['right_con'](len,l,x_coor,tlen)/2;
						react -= y-shear(x_coor);
						console.log(-shear(x_coor));
						arr.push({
							xstep:x_coor-tlen,
							index:i,
							yreact:react,
							// shear: -shear(x_coor)
							// arm: momentArm['right_con'](len,l,x_coor,tlen),
						});
					} //right con
		        },c_point:function(i,y,l,len,tlen){
					// if(tlen === undefined){
					// 	react -= y - areaForce['c_point'](len,l,0,0);
					// 	moment -= react * len/2;
					// 	arr.push({
					// 		xstep:len/2,
					// 		index:i,
					...
					// 	});
					// } else 
					if(x_coor<tlen+(len/2)){
						// console.log(i,y,l,len,tlen,'first');
						// console.log(react,'before');
						react -= y - areaForce['c_point'](len,l,x_coor,tlen);
						// console.log(react,'after',y,-areaForce['c_point'](len,l,x_coor,tlen));
						moment -= react * (x_coor-tlen); //change
						arr.push({
							xstep:x_coor-tlen,
							index:i,
							yreact:react,
						...
						// console.log(react,'before');
						react -= y - areaForce['c_point'](len,l,0,0); //Problem here //semi solved
						// console.log(react,'after',y,-areaForce['c_point'](len,l,0,0));
						moment -= react * len/2;
						arr.push({
							xstep:len/2,
							index:i,
						...
						// console.log(react,'after',y,- areaForce['c_point'](len,l,x_coor,tlen));
						moment -= react * (x_coor-(tlen+(len/2))); 
						arr.push({
		        ...
		        },two_point:function(i,y,l,len,tlen){
		        	if(tlen === undefined){
		        		react -= y - areaForce['c_point'](len,l,0,0);
		        		moment -= react * len/3;
			        	arr.push({
						...
						react -= -areaForce['c_point'](len,l,len/2,0);
						moment -= react * len/3;
			        	arr.push({
							xstep:len/3,
							index:i,
						...
						react -= y - areaForce['two_point'](len,l,x_coor,tlen);
						moment -= react * (x_coor-tlen); 
						arr.push({
							xstep: x_coor-tlen,
							index:i,
						...
						moment -= react * (x_coor-(tlen+(len/3))); //change
						arr.push({
						...
						moment -= react * (x_coor-(tlen+(len*2/3))); //change
						arr.push({
		        ...
		        },noload:function(i,y,l,len,tlen){ //Problem.. skip!
		        	console.log('noload reaction');
					if(tlen === undefined){
						console.log(react,'before',react-y);
						react -= y - (areaForce['noload'](len,l,len,0)); //sayop na biya ni. :(
						// react -= y - (y/2) //ni curve siya.. care
						console.log(shear(tlen));
						console.log(react,'after',y,- areaForce['noload'](len,l,len,0),momentArm['noload'](len,l,len,0));
						// moment -= react * len;
						arr.push({
							xstep:len,
							index:i,
							yreact:react,
							shear: -shearb4elm,
							tentMoment: -shearb4elm*len
							// arm: momentArm['noload'](len,l,len,0),
						});
					} else {
						console.log(react,'before',react-y);
						react -= y - areaForce['noload'](len,l,x_coor,tlen);
						// react -= y - ((y*(x_coor-tlen)/len)/2) //curves
						console.log(react,'after',y,- areaForce['noload'](len,l,x_coor,tlen),momentArm['noload'](len,l,x_coor,tlen));
						// moment -= react * (x_coor-tlen);
						console.log(shear(x_coor));
						arr.push({
							xstep:x_coor-tlen,
							...
							yreact:react,
							shear:-shear(x_coor),
							tentMoment: -shear(x_coor)*(x_coor-tlen)
						});
					}
		        } //noload
			} //just helpers
			...
			$(members).each(function(ind,elm){
				if(x_coor>=tlength+elm.eil.lenInMeters){
					pushBy[elm.load.type](ind,allReact[ind].y,elm.load.load,elm.eil.lenInMeters);
					tlength+=elm.eil.lenInMeters;
				} else {
					// console.log('called instead');
					pushBy[elm.load.type](ind,allReact[ind].y,elm.load.load,elm.eil.lenInMeters,tlength);
					return false;
				}
			});
		...
		function moment(){
			...
			$(dMArr).each(function(ind,elm){
				...
				if(elm.shear !== undefined){
					// console.log('momentcon1');
					total -= (elm.shear * elm.xstep)
				} else {
					// console.log('momentcon2');
					total -= (elm.yreact * elm.xstep) 
				}
				// total -= elm.moment; //as direct total
				// reactArr = elm.endmoment;
		...
		function deflect(){ //edit
			return (moment() * 1000000) / (memTemp.eil.elast * memTemp.eil.iner);
		}
		function bend(){
			var bend = {
				centriod:0,
			...
			if(memTemp.msc.material.shape ==='basic'){
				...
				x = (memTemp.msc.material.shape ==='square' ? memTemp.msc.material.dimensions.Base : memTemp.msc.material.dimensions.Height);
				bend.top = (((moment() * 1000000) * (x - memTemp.msc.centriod)) / memTemp.eil.iner) / 1000;
				bend.bottom = (((moment() * 1000000) * memTemp.msc.centriod) / memTemp.eil.iner) / 1000;
		...
		function shearStr(){ //add
			var strs = new Array(shearLimit); //remove later.. check shearbendCalc.js for sub correct
			var memTemp = findMember(x_coor);
			if(memTemp.msc.material.shape ==='basic'){
			...
			getWidth = {
				...
				circle:function(x){
					if(x>dim.Diameter/2){
						return dim.Diameter/2;
					} else if(x===dim.Diameter){
						return dim.Diameter;
					} else if(x<dim.Diameter/2){
						return dim.Diameter/2;
					}
				},
			}

			var s = shear() * 1000; //kN -> N
			VoverIB = s / (memTemp.eil.iner*dim.Base); // V/I

			var temp = [];
			var step = ((memTemp.msc.material.shape === 'square') ? dim.Base : dim.Height)/(shearLimit-1); 
			for(var i = 0;i<strs.length;i++){
				temp.push({
					index: 0,
					step: step*i,
					width: getWidth[memTemp.msc.material.shape](step*i)
				});
			}
			// console.log(temp);
			...
			for (var i = 0; i < strs.length; i++) {
				...
				strs[i] = VoverIB * temp[strs.length-1 - i].width * 1000000;  //Sayop jd ata diri.
				strs[i] /= 1000;
				// N/mm2 -> N/m2 = pascal ->kP
			}
