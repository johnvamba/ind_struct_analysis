...
function Beam() {
	...
	this.msc = function(){
		...
		function inspectReactions(){ //revise please. does nothing.
	...
	this.stressesOn = function(x_coor){
			...
			*/
			equally:function(l,ld,x,x2){
				...
				if(x2 == 0 && x == l){
					...
					// area= (l*ld)/2 - ((l-(x-x2))*(ld*((x-x2)/l)))/2;
					area = ((ld*l)/2)-(((l+x2-x)*(ld-(ld*((x-x2)/l))))/2);// type 3; basin sakto ni.
					// console.log('areashear',area);
					return area;
				}
			...
			$(Yreact).each(function(ind,elm){
				...
				if(c !== undefined){
					if(c.load.type !== 'noload'){
						if (x_coor>c.eil.lenInMeters+tLength) 
							total -= areaForce[c.load.type](c.eil.lenInMeters,c.load.load,c.eil.lenInMeters,0);
						else 
							total -= areaForce[c.load.type](c.eil.lenInMeters,c.load.load,x_coor,tLength);
					}
		...
		function moment(){
			...
			var force,arm,reacts = 0;
			$(allReact).each(function(ind,elm){
				if(tLength>x_coor) 
					return total;
				if(ind===0){
					total -= elm.r; //obj.r
				...
				momentb4 = total;
				// console.log(ind, 'semi before steps',x_coor,'total: '+total,arm,force);
				console.log('before',total);
				if(members[ind] !== undefined){
					var c = members[ind];
					// console.log('bfore',total);
					// reacts += elm.y;
					total -= (-elm.y * (x_coor-tLength));
					console.log('step',total,(-elm.y * (x_coor-tLength)));

					// console.log('subtotal',total,(-elm.y * (x_coor-tLength)),-elm.y,x_coor-tLength)
					if(c.load.type !== 'noload'){
					}
						// force = areaForce[c.load.type](c.eil.lenInMeters,c.load.load,x_coor,tLength);
						// arm = momentArm[c.load.type](c.eil.lenInMeters,c.load.load,x_coor,tLength);
						// total -= (force * arm); 
					// console.log('mid values',arm,force,-(force * arm), total);
					tLength += c.eil.lenInMeters;
				}
				// console.log('after steps',ind,total,arm,force);
			});
			// console.log('finalvalues: fForce,arm', total.toFixed(2),arm.toFixed(2),'aForce',force.toFixed(2),'step',(-force * arm).toFixed(2));
			total = parseFloat(total.toFixed(2));
			return total;
		}//returns kN-m
